...
def fir_filter(signal, Fs, cutoff, type='bandpass', width=0.2, ripple=65):
    ...
    if numtaps > len(signal[0]) / 3.5:
        numtaps = int(len(signal[0]) / 3.5)
    ...
    print(filter_coefs_peak)
    return filtfilt(filter_coefs_peak, 1, signal)


if __name__ == "__main__":
    a = np.random.random([1,500])
    b = a+np.random.random([1,500])
    c = np.random.random([1,500])
    d = c+np.random.random([1,500])
